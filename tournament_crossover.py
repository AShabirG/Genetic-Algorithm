...
while n < 90:
    ...
    while not optimal:
        ...
        while j < 20:
            ...
            # Choose 5 random genotypes and find the genotype with the maximum fitness amongst them
            row_a = np.where(fitnesses[deme_a] == max(random.sample(fitnesses[deme_a], 5)))
            row_b = np.where(fitnesses[deme_b] == max(random.sample(fitnesses[deme_b], 5)))

            # Migrate the genotype and mutate it
            new_gen[deme_a][1] = mutation(np.copy(demes[deme_b][row_a[0][0]]), n)
            new_gen[deme_b][1] = mutation(np.copy(demes[deme_a][row_b[0][0]]), n)

            j += 2

        for l in range(len(demes)):  # number of demes
            for i in range(2, 20):  # number of blank rows
                # Tournament selection of remainder of genotypes
                parent_1 = np.where(fitnesses[l] == max(random.sample(fitnesses[l], 5)))
                parent_2 = np.where(fitnesses[l] == max(random.sample(fitnesses[l], 5)))
                new_gen[l][i] = mutation(
                    crossover(np.copy(demes[l][parent_1[0][0]]), np.copy(demes[l][parent_2[0][0]])), n)
